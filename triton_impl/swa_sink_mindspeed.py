...
# NOTE: copied VERBATIM from MindSpeed-LLM g2_attention_kernel.py (Apache-2.0, Huawei/Ascend), with
# only the megatron `get_args` coupling removed so it imports standalone. The Triton kernels and
# SparseFlashAttentionTriton are UNCHANGED — we drive them dense (identity topk) from the adapter at
# the bottom of this file. See reference_from_mindspeed/README.md.


class G2CoreAttention(nn.Module):
    def __init__(self, config=None, layer_number=None, attn_mask_type=None, attention_type=None,
                 cp_comm_type=None, use_triton_sfa=True):
        super().__init__()
        self.use_triton_sfa = use_triton_sfa
        self.mtp_idx = None
...
# ===========================================================================
# ADAPTER (ours): drive the UNCHANGED MindSpeed sparse kernel DENSE for the DSpark block shape.
# The kernel is K==V (single latent for both key and value; confirmed for DSpark). We feed an
# IDENTITY topk (all KV keys 0..KV-1, then -1 padding up to a CONFIG_MAP topk) so the "sparse"
# gather selects every key -> dense attention over each block's KV latent. No kernel edits.
#   ours q [N,BS,H,D]; kv latent [N,KV,D] (K==V); sink [H]  ->  o [N,BS,H,D]
# MindSpeed layout: q [Seq,Batch,Head,Dim]=[BS,N,H,D]; kv [Seq_kv,Batch,Dim]=[KV,N,D];
#                   topk_idxs [Seq,Batch,TopK]; grid=(Batch=N, Seq=BS). bf16 only (kernel buffers).
# ===========================================================================
def _ms_pick_topk(kv_len):
    for t in sorted(CONFIG_MAP):
        if t >= kv_len:
            return t
    raise ValueError(f"KV={kv_len} exceeds largest CONFIG_MAP topk {max(CONFIG_MAP)}; add a config.")


def mindspeed_dense_topk(kv_len, BS, N, device):
    """Identity topk [Seq=BS, Batch=N, TOPK]: keys 0..kv_len-1 (every key), -1 padded to TOPK."""
    topk = _ms_pick_topk(kv_len)
    idx = torch.full((BS, N, topk), -1, device=device, dtype=torch.int32)
    idx[:, :, :kv_len] = torch.arange(kv_len, device=device, dtype=torch.int32).view(1, 1, kv_len)
    return idx


def mindspeed_dense_sink_attn(q, kv, sink, scale, topk_idxs=None):
    """Autograd (fwd+bwd) DENSE MLA block attention via the unchanged MindSpeed kernel (K==V).
    q [N,BS,H,D]; kv latent [N,KV,D] (used as BOTH key and value); sink [H]. Returns o [N,BS,H,D]."""
    N, BS, H, D = q.shape
    KV = kv.shape[1]
    q_ms = q.permute(1, 0, 2, 3).contiguous()          # [Seq=BS, Batch=N, Head=H, Dim=D]
    kv_ms = kv.permute(1, 0, 2).contiguous()           # [Seq_kv=KV, Batch=N, Dim=D]
    if topk_idxs is None:
        topk_idxs = mindspeed_dense_topk(KV, BS, N, q.device)
    o_ms = SparseFlashAttentionTriton.apply(q_ms, kv_ms, sink.float(), topk_idxs, scale)  # [BS,N,H,D]
    return o_ms.permute(1, 0, 2, 3).contiguous()       # [N,BS,H,D]
